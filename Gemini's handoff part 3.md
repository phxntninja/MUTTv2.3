MUTT Moog Forwarder Service Handoff

This document contains the complete source code and operational documentation for the MUTT Moog Forwarder Service (moog_forwarder_service.py).

1. Service Role

As defined in the project overview, the Moog Forwarder Service is Component #3 of the MUTT architecture (The "Forwarder").

It is a long-running, multi-threaded worker service that:

Pulls formatted alert payloads from the alert_queue using the reliable BRPOPLPUSH pattern.

Implements a global, shared rate limiter using a Redis Lua script to avoid overwhelming the Moogsoft API.

Forwards alerts to the Moogsoft API endpoint.

Implements an exponential backoff retry mechanism for failed requests (e.g., 5xx errors, timeouts).

Moves alerts to the moog_dlq (Dead Letter Queue) if they fail permanently (e.g., 4xx errors) or exhaust all retries.

2. Source Code (moog_forwarder_service.py)

Below is the complete, production-ready source code for moog_forwarder_service.py (v2.3).
...
'[https://moogsoft.example.com/api/v1/events](https://moogsoft.example.com/api/v1/events)')
...
def process_alert(alert_string, config, secrets, redis_client):
    ...
    try:
        ...
        return alert_string  # Discard malformed JSON - LREM
    ...
    if retry_count > config.MOOG_MAX_RETRIES:
        ...
        return None  # Done with this message (will be LREM'd)
    ...
    if not check_rate_limit(redis_client, config):
        ...
        # Re-queue to the front of the alert queue
        redis_client.lpush(config.ALERT_QUEUE_NAME, alert_string)
...
def main():
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            # Returns None on failure (re-queued, DLQ, or rate limited)
            result = process_alert(alert_string, config, secrets, redis_client)

            # --- Clean up the processing list ---
            # We LREM *no matter what* because process_alert() handles all
            # re-queueing and DLQ logic internally.
            ...
            try:
                ...
                sys.exit(1)
            # Message is still in processing_list, janitor will recover
...
This documentation was included with the source file and describes key features, deployment, and testing for this specific service.
...
17. ✅ Exponential backoff: 1s → 2s → 4s → 8s → 16s → (capped by max retries)
...
# Moogsoft
MOOG_WEBHOOK_URL=[https://moogsoft.example.com/api/v1/events](https://moogsoft.example.com/api/v1/events)
...
# Vault
VAULT_ADDR=[https://vault.prod.svc:8200](https://vault.prod.svc:8200)
