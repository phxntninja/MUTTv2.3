MUTT Web UI & API Service Handoff

This document contains the complete source code and operational documentation for the MUTT Web UI & API Service (web_ui_service.py).

1. Service Role

As defined in the project overview, this service is Component #4 of the MUTT architecture (The "Dashboard"). It is a Flask-based web application that serves several purposes:

Real-time Dashboard: Provides a simple HTML dashboard (at /) with live-updating charts showing event processing rates.

Prometheus Metrics: Exposes all collected Prometheus metrics (from all services, assuming a shared registry) at the /metrics endpoint for scraping.

Health Check: Provides a /health endpoint for load balancers and Kubernetes.

Management API: Exposes a secure, API-key-protected REST API (at /api/v1/*) for managing the system's configuration, which is stored in the PostgreSQL database. This includes CRUD (Create, Read, Update, Delete) operations for:

Alert Rules

Development Hosts

Device Team Mappings

Log Viewer API: Provides an API for viewing and paginating the event_audit_log table.

2. Source Code (web_ui_service.py)

Below is the complete, production-ready source code for web_ui_service.py (v2.3).
...
<head>
    ...
    <script src="[https://cdn.jsdelivr.net/npm/chart.js@3.7.1/dist/chart.min.js](https://cdn.jsdelivr.net/npm/chart.js@3.7.1/dist/chart.min.js)"></script>
    <style>
        :root {
            --bg-color: #1a1a1a;
            --card-color: #2c2c2c;
...
3. Operational Documentation (v2.3)

This documentation was included with the source file and describes key features, API endpoints, and testing for this specific service.

Key Improvements in v2.3

✅ All Critical Bugs Fixed

✅ Fixed __name__ syntax errors.

✅ Fixed Chart.js CDN URL.

✅ Fixed JavaScript template literal.

✅ Removed duplicate Prometheus registration (metrics auto-register).

✅ Added proper /metrics endpoint.

✅ Security & Performance
6.  ✅ API key authentication with constant-time comparison.
7.  ✅ Metrics caching (5s TTL) to reduce Redis load.
8.  ✅ PostgreSQL connection pooling.
9.  ✅ Proper error handling with safe_int().

✅ Complete CRUD Functionality
10. ✅ Alert Rules API: GET, POST, PUT, DELETE.
11. ✅ Audit Logs API: Paginated with filtering.
12. ✅ Dev Hosts API: GET, POST, DELETE.
13. ✅ Device Teams API: GET, POST, PUT, DELETE.
...
Query (for dashboard): ?api_key=your-api-key
