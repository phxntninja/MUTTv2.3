# MUTT Standalone Server Installation Guide (Ubuntu 22.04)
...
Before installing any new software, ensure your Ubuntu system has the latest security patches and package updates. This command refreshes the package index and upgrades all installed packages to their latest versions.
...
MUTT requires Python 3.10, along with tools for building Python packages and PostgreSQL client libraries. This step installs all necessary system dependencies including:
- **python3.10** and **python3.10-venv**: Python runtime and virtual environment support
- **build-essential**: C/C++ compilers needed for building Python packages with native extensions
...
```bash
sudo apt -y install python3.10 python3.10-venv python3-pip build-essential libpq-dev rsyslog curl ufw
```

**Verification:** Run `python3.10 --version` to confirm Python 3.10 is installed.

### 2.3. Create Service User

For security best practices, MUTT services should run under a dedicated, non-privileged system account named `mutt`. This user will have no login shell and limited privileges, reducing the attack surface if a service is compromised. The `|| true` ensures the command succeeds even if the user already exists.

```bash
sudo groupadd --system mutt || true && sudo useradd --system --home-dir /opt/mutt --create-home --shell /usr/sbin/nologin --gid mutt mutt || true
```
...
# scp -r /path/to/mutt/source/* user@ubuntu-server:/tmp/mutt-code/
# Then on the server:
...
```bash
sudo -u mutt -H /usr/bin/python3.10 -m venv /opt/mutt/venv && sudo -u mutt -H /opt/mutt/venv/bin/pip install --upgrade pip wheel setuptools
```
...
Create the file `/etc/mutt/mutt.env` using your preferred text editor (nano, vim, etc.) and paste the template below. **Important:** Replace all placeholder values (e.g., `your-redis-host`, `your-postgres-host`, `your-vault-url`) with the actual values for your environment before starting the services.
...
```bash
sudo apt -y install snmp snmptrapd
```
...
Option B: Direct HTTP POST to MUTT (traphandle)
...
[Service]
Environment=MUTT_INGEST_URL=http://127.0.0.1:8080/ingest
...
sudo bash -c 'cat > /etc/nginx/sites-available/mutt_ingestor.conf <<EOF
server {
    listen 8443 ssl;
    server_name _;
...
EOF'
sudo ln -s /etc/nginx/sites-available/mutt_ingestor.conf /etc/nginx/sites-enabled/mutt_ingestor.conf || true
...
Configure Ubuntu's Uncomplicated Firewall (UFW) to allow inbound traffic to the MUTT service ports. This step is critical for allowing external systems to send logs to the Ingestor and for accessing the Web UI. Adjust the firewall rules based on your network requirements:
...
```bash
sudo ufw allow 8080/tcp    # Ingestor HTTP
sudo ufw allow 8081/tcp    # Alerter metrics
sudo ufw allow 8082/tcp    # Alerter health
sudo ufw allow 8083/tcp    # Moog metrics
sudo ufw allow 8084/tcp    # Moog health
sudo ufw allow 8090/tcp    # Web UI
sudo ufw allow 514/tcp     # Syslog (if needed)
sudo ufw allow 514/udp     # Syslog (if needed)
sudo ufw allow 162/udp     # SNMP traps (if needed)
sudo ufw --force enable
```

**Verification:** Check firewall status with `sudo ufw status verbose` to confirm all rules are active and the firewall is enabled.
...
Congratulations! You have successfully deployed MUTT v2.5 on your Ubuntu 22.04 server. All four core services (Ingestor, Alerter, Moog Forwarder, and Web UI) are now running under systemd supervision with automatic restart and boot-time startup enabled.
