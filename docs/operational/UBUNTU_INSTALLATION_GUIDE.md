...
**[Claude: Write a brief introduction explaining the purpose of this document. Mention that it provides step-by-step instructions to deploy the MUTT v2.5 application on a standalone Ubuntu 22.04 server.]**

This guide assumes you are starting with a fresh Ubuntu 22.04 server and have `sudo` privileges. It also assumes that the external dependencies (PostgreSQL, Redis, Vault) are already set up and network-accessible.
...
**[CODEX: Provide the `apt` command to update system packages.]**

### 2.2. Install Dependencies

**[Claude: Explain that MUTT requires Python, a virtual environment tool, and other build tools to be installed.]**

**[CODEX: Provide the single `apt` command to install `python3.10`, `python3.10-venv`, `python3-pip`, `build-essential`, and `libpq-dev`.]**
...
**[CODEX: Provide the `ufw` commands to allow traffic on TCP ports 8080, 8081, 8082, 8083, 8084, 8090, 514 and UDP ports 514, 162. Include the command to enable `ufw`.]**
