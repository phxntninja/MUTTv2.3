...
- Executive and Technical Reveal.js decks:
  ...
  - These files load Reveal.js from a CDN (internet required). For offline use, consider “Offline Mode” below.
...
- Serve locally to avoid file path issues or to vendor libraries if needed:
  - Python: python -m http.server 8089 (then browse to http://localhost:8089/docs/slides/...)
  - Node: npx http-server docs/slides -p 8089
- To fully offline the Reveal.js decks, vendor Reveal.js assets locally and update the <link>/<script> tags to local paths.
